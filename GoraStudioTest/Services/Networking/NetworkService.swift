...
protocol NetworkServiceProtocol {
    ...
    func request(url: String, completion: @escaping (Result<Data?,Error>) -> ())
}

struct NetworkService: NetworkServiceProtocol {
    
    public func request(url: URL, completion: @escaping (Result<Data?, Error>) -> ()) {
        let task = createDataTask(from: url, completion: completion)
        task.resume()
    }
    
    public func request(url: String, completion: @escaping (Result<Data?, Error>) -> ()) {
        guard let url = URL(string: url) else { return }
        request(url: url, completion: completion)
    }
    
    private func createDataTask(from url: URL, completion: @escaping (Result<Data?,Error>) -> ()) -> URLSessionDataTask {
